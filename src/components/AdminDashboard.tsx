...
export default function AdminDashboard({ token }: { token: string }) {
  ...
  useEffect(() => {
    fetch("http://localhost:4000/progress/summary", {
  ...
  if (!data) return <div className="p-10">Loading Admin Metrics...</div>;
    ...
    <div className="p-8 bg-gray-50 min-h-screen">
      <div className="max-w-6xl mx-auto">
        <h1 className="text-3xl font-black text-gray-900 mb-8 flex items-center gap-3">
          <BarChart3 className="text-indigo-600" /> Instructor Analytics
        </h1>

        <div className="grid grid-cols-3 gap-6 mb-8">
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
            <Users className="text-indigo-500 mb-2" />
            <div className="text-sm text-gray-500">Avg. Problems Solved</div>
            <div className="text-2xl font-bold">{Math.round(data.averages.avg_solved)}</div>
          </div>
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
            <Award className="text-emerald-500 mb-2" />
            <div className="text-sm text-gray-500">Avg. Success Rate</div>
            <div className="text-2xl font-bold">{Math.round(data.averages.avg_success * 10)}%</div>
          </div>
        </div>
          ...
          <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
            ...
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={data.skillDistribution} dataKey="count" nameKey="level" cx="50%" cy="50%" outerRadius={80} label>
