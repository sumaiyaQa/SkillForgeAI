...
import { loadProblems, getProblemDatabase, type Problem } from './utils/problemDatabase';
...
const App: React.FC = () => {
  ...
  const recommendedProblems = useMemo(() => {
    return [...getProblemDatabase()].sort((a, b) => {
      const scoreA =
  ...
  useEffect(() => {
    if (!authUser) return;
    loadProblems(authUser.token).then(problems => {
      ...
      fetch('http://localhost:4000/progress', {
        ...
        .then(data => {
          if (data?.profile?.skillLevel) {
            setUserProfile(prev => ({ ...prev, ...data.profile, solvedProblemIds: data.profile.solvedProblemIds ?? [] }));
          }
                    ...
                    {recommendedProblems.map(p => {
                      ...
                      return (
                        <button
                          ...
                          className={`w-full text-left p-3 rounded-lg border flex justify-between items-center transition-colors ${isActive
                        ...
                        >
                          <div>
                            ...
                            <div
                              className={`text-[10px] uppercase font-bold mt-0.5 ${p.difficulty === 'hard'
                                ? 'text-red-500'
                                : p.difficulty === 'medium'
                                  ? 'text-amber-500'
                                  : 'text-emerald-500'
                        ...
                        .map(([concept, value]) => (
                          <div key={concept}>
                            ...
                            <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                              <div
                                className={`h-full rounded-full transition-all duration-500 ${value >= 0.75
                                  ? 'bg-emerald-500'
                                  : value >= 0.5
                                    ? 'bg-amber-400'
                                    : 'bg-red-400'
                  ...
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <span
                          className={`text-xs font-bold uppercase px-2 py-0.5 rounded-full ${currentProblem.difficulty === 'hard'
                  ...
                  <div className="bg-gray-50 border-b px-4 flex">
                    {(['code', 'visualization', 'analysis'] as const).map(tab => (
                      <button
                        ...
                        className={`px-6 py-3 text-xs font-bold uppercase border-b-2 transition-colors ${activeTab === tab
                      ...
                      <>
                        ...
                        {(output || error) && (
                          <div className="mt-4">
                            ...
                            <div
                              className={`p-4 font-mono text-xs shadow-inner ${isSolved ? 'rounded-b-lg' : 'rounded-lg'
                                } ${error
                              ...
                              <div className="flex gap-1">
                                {[1, 2, 3, 4, 5].map(n => (
                                  <button
                                    ...
                                    className={`w-8 h-8 rounded-full text-sm font-bold border transition-colors ${feedbackRating === n
